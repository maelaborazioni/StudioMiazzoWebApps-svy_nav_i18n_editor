...
function resetToDefault(event) {
	...
	if (globals.svy_mod_dialogs_global_showQuestionDialog(null, i18n.getI18NMessage('svy.fr.dlg.reset_i18n'), ok, no) == ok) {
...
function onShowForm(_firstShow, _event) {	
	...
	} else if (globals.nav.mode == 'browse') {
...
function dc_new(_event, _triggerForm) {
	globals.svy_mod_dialogs_global_showErrorDialog(null, i18n.getI18NMessage('svy.dlg.addingRecordNotAllowed'), i18n.getI18NMessage('svy.fr.lbl.ok'));
}
...
function dc_delete(_event, _triggerForm) {
	globals.svy_mod_dialogs_global_showErrorDialog(null, i18n.getI18NMessage('svy.dlg.deletingRecordNotAllowed'), i18n.getI18NMessage('svy.fr.lbl.ok'));
}
